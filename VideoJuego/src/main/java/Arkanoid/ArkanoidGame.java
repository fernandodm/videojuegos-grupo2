...
import componentes.ladrillos.LadrilloEspecial;
...
public class ArkanoidGame extends Game {
	...
	public GameScene buildArkanoidScene() {
		...
		Raqueta raqueta = new Raqueta(200, 450, 50, 4, Color.BLACK, 500, 0,
				this.getDisplayWidth());
		Marcador marcador = new Marcador(50, this.getDisplayHeight() - 35,
				Color.GRAY);
		Pelota pelota = new Pelota(20, 200, 430, new Vector(5, 5), 350,
				raqueta, marcador);
		...
		ArrayList<Ladrillo> ladrillos = renderElements(arkanoidScene, pelota,
				marcador, raqueta);
		arkanoidScene.setLadrillos(ladrillos);
		return arkanoidScene;
	}

	public ArrayList<Ladrillo> renderElements(final GameScene scene,
			final Pelota pelota, Marcador marcador, Raqueta raqueta) {
		...
		for (int y = 0; y < width; y++) {
			for (int x = 0; x < length; x++) {
				...
				if ((cantLadrillosEsp > 0 && ((int) (Math.random() * 100)) > 60)) {
					ladrillo = this.crearLadrilloEspecial(48, 18, x, y,
							marcador, raqueta, pelota);
					cantLadrillosEsp--;
				}
	...
	public Ladrillo crearLadrilloEspecial(int ancho, int alto, double x,
		...
		LadrilloEspecial[] ladrillos = {
				new LadrilloPaletaChica(Color.PINK, 48, 18, x, y, marcador,
						raqueta, pelota),
				new LadrilloPelotaRapida(Color.PINK, 48, 18, x, y, marcador,
						raqueta, pelota),
				new LadrilloPaletaGrande(Color.PINK, 48, 18, x, y, marcador,
						raqueta, pelota),
				new LadrilloPelotaLenta(Color.PINK, 48, 18, x, y, marcador,
						raqueta, pelota),
				new LadrilloPelotaRapida(Color.PINK, 48, 18, x, y, marcador,
						raqueta, pelota),
				new LadrilloDisparo(Color.PINK, 48, 18, x, y, marcador,
						raqueta, pelota),
				new LadrilloPaletaPegajosa(Color.PINK, 48, 18, x, y, marcador,
						raqueta, pelota) };
		return ladrillos[((int) (Math.random() * ladrillos.length))];
